...
pub const NAME: &str = "Orbital-Demo-Project: PingPong";
...
pub fn entrypoint(event_loop_result: Result<EventLoop<()>, EventLoopError>) {
    ...
    // Create both a Ping and a Pong element to demonstrate message passing
    let app = StandardApp::with_initial_elements(vec![
        Box::new(PingPongElement::new(true)),  // Ping
        Box::new(PingPongElement::new(false)), // Pong
    ]);
