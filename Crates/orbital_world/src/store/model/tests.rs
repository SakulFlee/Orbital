...
use orbital_resources::{MaterialDescriptor, MeshDescriptor, ModelDescriptor, Transform, Vertex};
...
fn test_base_model_despawning_with_instances() {
    ...
    let base_id = store.label_to_id("BaseCube").unwrap();
    assert_eq!(store.map_descriptors[&base_id].transforms.len(), 4);
...
fn test_different_materials_prevent_instancing() {
    ...
    let descriptor2 = ModelDescriptor {
        ...
        materials: vec![Arc::new(different_material)],
...
fn test_instance_label_generation() {
    ...
    assert_eq!(instance_labels[0], "Instance");
}
