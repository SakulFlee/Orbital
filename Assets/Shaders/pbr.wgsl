...
fn compute_shadow_for_light(world_pos: vec3<f32>, view_depth: f32, light_idx: u32, normal: vec3<f32>, fin: FragmentData) -> f32 {
    ...
    for (var i = 0u; i < shadow_data.cascade_count; i++) {
        ...
        if (slot.shadow_type == SHADOW_TYPE_DIRECTIONAL_CASCADE) {
            ...
            // Guard: fragments at/behind the near plane have clip_pos.w
            // near-zero or negative.  Dividing by it scrambles ndc and
            // causes false lit/shadow results.  Treat these as unshadowed.
            if (clip_pos.w <= 0.001) {
                return 1.0;
            }
            let ndc = clip_pos.xyz / clip_pos.w;
