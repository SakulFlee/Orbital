...
/// # Returns
/// A tuple (Tangent, Bitangent).
pub fn generate_sphere_tangent_frame(
    normal: Vector3<f32>,
    uv: Option<(f32, f32)>,
...
mod tests {
    ...
    fn test_sphere_tangent_frame_poles() {
        ...
        assert!(
            tangent.dot(north_pole).abs() < 1e-6,
            "Tangent should be perpendicular to normal"
        );
        assert!(
            bitangent.dot(north_pole).abs() < 1e-6,
        ...
        assert!(
            (tangent.x - 1.0).abs() < 1e-6,
            "North pole tangent should be in X direction"
        );
        assert!(
            tangent.y.abs() < 1e-6,
            "North pole tangent should have no Y component"
        );
        assert!(
            tangent.z.abs() < 1e-6,
            "North pole tangent should have no Z component"
        );
        ...
        // Verify orthogonality
        assert!(
            tangent.dot(south_pole).abs() < 1e-6,
            "Tangent should be perpendicular to normal"
        );
        assert!(
            bitangent.dot(south_pole).abs() < 1e-6,
            "Bitangent should be perpendicular to normal"
        );
        assert!(
            tangent.dot(bitangent).abs() < 1e-6,
        ...
        assert!(
            (tangent.x - 1.0).abs() < 1e-6,
            "South pole tangent should be in X direction"
        );
        assert!(
            tangent.y.abs() < 1e-6,
            "South pole tangent should have no Y component"
        );
        assert!(
            tangent.z.abs() < 1e-6,
            "South pole tangent should have no Z component"
        );
    }
    ...
    fn test_sphere_tangent_frame_equator() {
        ...
        // Verify orthogonality
        assert!(
            tangent.dot(equator_point).abs() < 1e-6,
            "Tangent should be perpendicular to normal"
        );
        assert!(
            bitangent.dot(equator_point).abs() < 1e-6,
            "Bitangent should be perpendicular to normal"
        );
        assert!(
            tangent.dot(bitangent).abs() < 1e-6,
        ...
        assert!(
            tangent.y.abs() < 1e-6,
            "Tangent at equator should be perpendicular to Y axis"
        );
    }
}
