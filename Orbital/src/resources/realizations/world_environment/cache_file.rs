...
use crate::{error::Error, resources::{descriptors::WorldEnvironmentDescriptor, realizations::Texture as OrbitalTexture}};
...
impl CacheFile {
    ...
    pub fn make_textures(&self, world_environment_descriptor: &WorldEnvironmentDescriptor, device: &Device, queue: &Queue) -> (OrbitalTexture, OrbitalTexture) {
        let cube_face_size = *match world_environment_descriptor {
            WorldEnvironmentDescriptor::FromFile { skybox_type: _, cube_face_size, path: _, sampling_type: _ } => cube_face_size,
            WorldEnvironmentDescriptor::FromData { skybox_type: _, cube_face_size, data: _ , size: _, sampling_type : _} => cube_face_size,
        };
